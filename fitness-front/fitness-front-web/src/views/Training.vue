<template>
  <div class="training">
    <div class="page-header">
      ...
      <el-button type="primary" size="large" @click="openSetup" class="start-btn">▶ 开始今日训练</el-button>
    </div>
    ...
    <el-row :gutter="16" class="stats-row">
      <el-col :xs="12" :sm="8" :md="4" v-for="s in stats" :key="s.label">
        <el-card shadow="hover" class="stat-card">
          <div class="stat-value" :style="{ color: s.color }">{{ s.displayValue }}</div>
    ...
    <!-- 日历热力图 -->
    <el-card shadow="hover" style="margin-top:16px" v-if="heatmapDays.length > 0">
    ...
    <!-- 快速录入 -->
    <el-card shadow="hover" style="margin-top:16px" id="add-form-card">
      <template #header><span>✚ 快速记录</span></template>
      <el-row :gutter="16">
        <el-col :sm="14" :xs="24">
          <el-form :inline="true" :model="addForm" size="default" class="add-form">
            <el-form-item label="日期"><el-date-picker v-model="addForm.recordDate" type="date" value-format="YYYY-MM-DD" style="width:130px" size="small" /></el-form-item>
            <el-form-item label="类型"><el-select v-model="addForm.trainingType" placeholder="训练类型" size="small" style="width:130px" clearable><el-option v-for="t in trainingTypes" :key="t.value" :label="t.icon+' '+t.label" :value="t.value" /></el-select></el-form-item>
            ...
            <el-form-item label="时长"><el-input-number v-model="addForm.duration" :min="1" :max="300" style="width:85px" size="small" /></el-form-item>
            <el-form-item label="热量"><el-input-number v-model="addForm.calories" :min="0" :max="5000" style="width:95px" size="small" /></el-form-item>
            <el-form-item label="备注"><el-input v-model="addForm.notes" placeholder="训练内容" style="width:130px" size="small" /></el-form-item>
    ...
    <el-card shadow="hover" style="margin-top:16px">
      <template #header>
        <div class="table-header">
          ...
          <div class="table-header-right">
            <el-date-picker v-model="dateRange" type="daterange" range-separator="~" start-placeholder="开始" end-placeholder="结束" value-format="YYYY-MM-DD" size="small" style="width:220px" @change="page=1;fetchRecords()" unlink-panels />
            <span class="table-count" v-if="total">共 {{ total }} 条</span>
          </div>
        </div>
      </template>
      ...
      <div class="pagination-wrap"><el-pagination v-model:current-page="page" v-model:page-size="size" :page-sizes="[10,20,50]" :total="total" layout="total, sizes, prev, pager, next" @current-change="fetchRecords" @size-change="fetchRecords" /></div>
    </el-card>
    ...
    <!-- 训练设置对话框 -->
    <el-dialog v-model="setupVisible" title="训练设置" width="620px" @closed="resetSetup">
      ...
      <div class="setup-selected" v-if="setup.exercises.length > 0">
        ...
        <div v-for="(ex, i) in setup.exercises" :key="i" class="setup-ex-row">
          ...
          <span class="setup-ex-controls">
            <el-input-number v-model="ex.sets" :min="1" :max="12" size="small" style="width:60px" />
            <span class="ctrl-label">组</span>
            <el-input-number v-model="ex.restSeconds" :min="10" :max="300" :step="5" size="small" style="width:70px" />
...
const streak = ref(0), heatmapDays = ref([])
...
const stats = computed(()=>[
  ...
  { label:'总热量(kcal)',value:records.value.reduce((s,r)=>s+(r.calories||0),0),displayValue:statDisplays.cal,color:'#e6a23c'},
...
// Muscle group keyword mapping — titles are English, match actual exercise names
...
.start-btn { font-size:15px; padding:12px 28px; font-weight:600; }
.stats-row { margin-bottom:16px; }
.heatmap-header { display:flex; align-items:center; gap:12px; margin-bottom:12px; }
.heatmap-title { font-weight:600; font-size:14px; }
.heatmap-streak { font-size:13px; color:var(--color-brand); font-weight:600; }
.heatmap-grid { display:flex; gap:4px; flex-wrap:wrap; }
.heatmap-cell { width:28px; height:28px; border-radius:3px; background:var(--color-bg-light); border:1px solid var(--color-border-light); transition:all var(--transition-fast); }
.heatmap-cell.trained { background:#67c23a; border-color:#52b818; }
.heatmap-cell.today { box-shadow:0 0 0 2px var(--color-brand); }
.heatmap-legend { display:flex; align-items:center; gap:6px; margin-top:8px; font-size:12px; color:var(--color-text-secondary); }
.legend-dot { width:12px; height:12px; border-radius:2px; }
.legend-dot.empty { background:var(--color-bg-light); border:1px solid var(--color-border-light); }
...
.timer-box { display:flex; flex-direction:column; align-items:center; gap:8px; padding:8px 0; }
.timer-display { font-size:36px; font-weight:700; font-variant-numeric:tabular-nums; color:var(--color-text-title); letter-spacing:2px; font-family:'SF Mono','Cascadia Code','Consolas',monospace; }
.timer-btns { display:flex; gap:8px; }
.table-header { display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:8px; }
.table-header-right { display:flex; align-items:center; gap:12px; }
.table-count { font-size:12px; color:var(--color-text-weak); }
...
.setup-ex-row { display:flex; align-items:center; gap:12px; padding:8px 0; border-bottom:1px solid var(--color-border-light); }
.setup-ex-name { flex:1; font-weight:500; min-width:100px; }
.setup-ex-controls { display:flex; align-items:center; gap:4px; flex-shrink:0; }
.ctrl-label { font-size:11px; color:var(--color-text-secondary); margin:0 4px 0 0; }
...
@media (max-width:767px) { .timer-display { font-size:28px; } .heatmap-cell { width:22px; height:22px; } }
</style>
